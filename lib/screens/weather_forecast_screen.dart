...
class _WeatherForecastScreenState extends State<WeatherForecastScreen> {
  ...
  Future<void> _fetchLocationAndWeather() async {
    ...
    try {
      ...
      if (_currentWeather != null) {
        final weatherCode =
            (_currentWeather!['weather_code'] as num?)?.toInt() ?? 0;
        print(
          'Current weather code from API: $weatherCode - ${WeatherService.getWeatherInfoFromCode(weatherCode)['description']}',
        );
      }
    } catch (e) {
  ...
  Widget _buildWeatherSkeleton() {
    ...
    final containerBackgroundColor = const Color(
      0xFF1A2533,
    ); // Dark blue-grey background
    final baseSkeletonColor = const Color(
      0xFF28384A,
    ); // Slightly lighter base blue-grey
    final highlightSkeletonColor = const Color(
      0xFF5A7698,
    ); // Much lighter blue-grey highlight for more pop
    final cardSkeletonColor = const Color(
      0xFF202D3D,
    ); // Slightly darker card background
    final placeholderShapeColor = const Color(
      0xFF5A7698,
    ); // Match highlight color for shapes
                    ...
                    child: Row(
                      children: [
                        ...
                        Container(
                          ...
                          height: 18, // Taller temp
                          decoration: BoxDecoration(
                            color: placeholderShapeColor.withOpacity(
                              0.6,
                            ), // Adjusted dimmer opacity
                            borderRadius: BorderRadius.circular(
                              5,
                            ), // Adjusted radius
                          ),
  ...
  Widget build(BuildContext context) {
    ...
    final bool isNight =
        _currentWeather != null &&
        (_currentWeather!['is_day'] as num?)?.toInt() == 0;
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text(
            _isLoading ? 'Weather' : _locationName,
  ...
  Widget _buildCurrentWeather(BuildContext context, bool isDarkMode) {
    ...
    final weatherInfo = WeatherService.getWeatherInfoFromCode(
      weatherCode,
      isDay: isDay,
    );
    final iconUrl =
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              ...
              color:
                  isDarkMode ? Colors.grey[300] : Colors.black.withOpacity(0.8),
            ),
          ),
          ...
          precipitationWidgets.add(
            Row(
              ...
              children: [
                ...
                Text(
                  '${totalSnow.toStringAsFixed(1)}cm',
                  style: TextStyle(
                    fontSize: 11,
                    ...
                    Image.network(
                      ...
                      errorBuilder:
                          (context, error, stackTrace) => const Icon(
                            ...
                            size: 30,
                            color:
                                Colors.yellow, // Explicitly set color for sun
                          ),
                    ),
